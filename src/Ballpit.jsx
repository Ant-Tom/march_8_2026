...
class Z extends d {
  ...
  #S() {
    ...
    const firstColor = Array.isArray(this.config.colors) && this.config.colors.length > 0
      ? new l(this.config.colors[0]).getHex()
      : 0;
    this.light = new u(firstColor, this.config.lightIntensity);
    this.add(this.light);
  }
  setColors(e) {
      ...
      const t = (function (e) {
        ...
        return {
          ...
          getColorAt: function (ratio, out = new l()) {
            const scaled = Math.max(0, Math.min(1, ratio)) * Math.max(0, t.length - 1);
            const idx = Math.min(Math.floor(scaled), t.length - 1);
            const start = i[idx];
